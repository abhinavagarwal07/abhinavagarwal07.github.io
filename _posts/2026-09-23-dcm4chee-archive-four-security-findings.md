...
The deletion path targets all matching image locations on one storage system; a caller able to enumerate storage IDs could repeat it across systems. A prior scan might still appear available in search results when its file can no longer be retrieved. Recovery depends on the site's backups. Accepted concurrent parser inputs could consume processing capacity, but the measured denial of service was limited to library threads and a synthetic worker pool. No hospital was tested, and I have no evidence of clinical harm or exploitation in the wild.

There is documented healthcare use, but no reliable public count of affected hospitals. [Vanderbilt University Medical Center in the U.S.](https://pmc.ncbi.nlm.nih.gov/articles/PMC13275161/) describes DCM4CHEE in its ImageVU **research** PACS, separate from its clinical PACS. A [Primaya Hospital Semarang study](https://ejournal.poltekkes-smg.ac.id/ojs/index.php/JAHMT/article/view/10886/0) describes an archive connected to CT and X-ray; a [2024 Catalan procurement annex](https://contractaciopublica.cat/portal-api/descarrega-document/300649001/4547195E130CBF2180DDA3C8B7322802) describes an IDI PACS based on dcm4chee while discussing migration. [Mesys](https://www.mesys.com.ar/) says its commercial PACS uses DCM4CHEE and lists Argentine hospital customers; [Comiere](https://comiere.com/products/) says its PACS is built on dcm4chee; [MedDream](https://meddream.com/products/meddream-dicom-viewer/open-source-dicom-viewer/) offers a commercial viewer integrated with dcm4chee v2/v5. None of these sources establishes a site's current version or exposure.

[NIST used DCM4CHEE](https://www.nccoe.nist.gov/publication/1800-24/VolC/index.html) in its 2020 PACS cybersecurity reference build. The [official archive Docker image](https://hub.docker.com/r/dcm4che/dcm4chee-arc-psql) has more than 10 million cumulative pulls; downloads and automated pulls are not installations or hospitals. Hospitals can also [deploy the archive directly](https://github.com/dcm4che/dcm4chee-arc-light/wiki/Running-on-Docker).

The parser code can travel beyond the archive. Source review found inbound multipart parsing in [Alvearie imaging-ingestion](https://github.com/Alvearie/imaging-ingestion/blob/90775a5912e0b79b846801b668aacba1a7f6fa8d/dicomweb-stow-service/src/main/java/org/alvearie/imaging/ingestion/service/stow/StowResource.java) and [dcm4chee-proxy](https://github.com/dcm4che/dcm4chee-proxy/blob/fb2be79bc02881ec8b214647cf958b80404b4b6e/dcm4chee-proxy-service/src/main/java/org/dcm4chee/proxy/stow/StowRS.java). [Weasis DICOM tools](https://github.com/nroduit/weasis-dicom-tools) and [818000/bus](https://github.com/818000/bus) contain copied JPEG parser logic in image-reading paths, so replacing an upstream JAR would not replace those copies. dcm4che artifacts are served from the project's [own Maven repository](https://www.dcm4che.org/maven2/), which a Maven Central-only inventory can miss. These are source-level reachability findings, not tests of deployed products; a dependency declaration alone does not establish a vulnerable path. I notified upstream through CERT/CC but cannot confirm that those downstream vendors were individually notified.
...
The mass-deletion advisory labels `5.35.2` as patched. At the checked [archive source commit](https://github.com/dcm4che/dcm4chee-arc-light/blob/51d2b6717b58c22a6a95906a426fea5144a08478/dcm4chee-arc-storage-ejb/src/main/java/org/dcm4chee/arc/storage/ejb/StorageEJB.java#L66-L72), however, `StorageEJB.updateStatus()` still selects the unrestricted query for `OK` to `TO_DELETE`; the narrower orphan-only query is selected for other destination statuses. That is a source-level concern, not a runtime test of a newer image. Check the [archive releases](https://github.com/dcm4che/dcm4chee-arc-light/releases), [library releases](https://github.com/dcm4che/dcm4che/releases), and the four advisories for current build and remediation information.

At publication, the latest GitHub release in both source repositories was `5.35.1`; neither had published a `5.35.2` release tag. Official Docker images labeled [`5.35.2`](https://hub.docker.com/v2/repositories/dcm4che/dcm4chee-arc-psql/tags/5.35.2) and [`5.35.2-secure`](https://hub.docker.com/v2/repositories/dcm4che/dcm4chee-arc-psql/tags/5.35.2-secure) were updated before the multipart fix. All four advisories listed no known CVE, so CVE-only feeds would miss them. A version label alone is not evidence that a tested fixed image was available.

Separately, three August advisories reported by other researchers describe server-takeover paths through [storage configuration](https://github.com/dcm4che/dcm4chee-arc-light/security/advisories/GHSA-rf29-xp3v-4wp3), [vendor-data ZIP extraction](https://github.com/dcm4che/dcm4chee-arc-light/security/advisories/GHSA-hxq8-xgh3-4f76), and [XSLT rules](https://github.com/dcm4che/dcm4chee-arc-light/security/advisories/GHSA-5j3q-rhfh-xvx8). They are distinct from the four findings above. CERT/CC coordination of my report continues under VU#288816.
